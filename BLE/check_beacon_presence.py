...
URL_HUBITAT = 'http://[HUB IP]/apps/api/[Maker API ID]/devices/PARAM_DEVICEID/PARAM_PRESENCE?access_token=[access-token]'
...
TAG_DATA = [
            ["Beacon_white","[BLE device MAC]",30,0,[Maker Api device ID],SWITCH_MODE],
            ["Beacon_blue","[BLE device MAC]",30,0,[Maker Api device ID],SWITCH_MODE]
           ]
...
ABSENCE_FREQUENCY=30  # frequency of the test of absence. in seconde. (without detection, switch "AWAY".
...
def request_thread(deviceID,presence):
    try:
        url = URL_HUBITAT
        url=url.replace('PARAM_DEVICEID',str(deviceID))
        url=url.replace('PARAM_PRESENCE',str(presence))
    ...
    def run(self):
        ...
        for tag in TAG_DATA:
            ...
            if elapsed_time_absence>=tag[2] : # sleep execute after the first Home check.
                ...
                threadReqAway = threading.Thread(target=request_thread,args=(tag[4],"departed"))
                threadReqAway.start()
                ...
                for i in range(0, num_reports):
                            ...
                            for tag in TAG_DATA:
                                if macAdressSeen.lower() == tag[1].lower():  # MAC ADDRESS
                                    ...
                                    if tag[5]==SWITCH_MODE and elapsed_time>=tag[2] : # Update only once : after an absence (>timeout). It's back again
                                        threadReqHome = threading.Thread(target=request_thread,args=(tag[4],"arrived"))
                                        threadReqHome.start()
                                        ...
                                        threadReqHome = threading.Thread(target=request_thread,args=(tag[4],rssi))
                                        threadReqHome.start()
